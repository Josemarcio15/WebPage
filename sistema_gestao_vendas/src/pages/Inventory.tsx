...
function Inventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        editId ? `${import.meta.env.VITE_API_URL}/api/products/${editId}` : `${import.meta.env.VITE_API_URL}/api/products`,
        {
  ...
  const confirmDelete = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/products/${productToDelete.id}`, { method: 'DELETE' });
      if (!res.ok) {
